 ...
 *
 * This is a drop-in replacement for FOSSBilling's stock ErrorPage. The error
 * code tables, category mapping and Sentry "report" flags are left untouched so
 * behaviour is identical; only the rendered HTML is replaced with the Quizontal
 * Cloud brand (logo, gradient, typography and a "Back to Quizontal Cloud" link).
 *
 * Install it with deploy/install-custom-error-page.sh, which backs up the
 * original file so it can be restored after a FOSSBilling upgrade.
 */
class ErrorPage
{
    /**
     ...
     *
     * You can edit this line directly in DirectAdmin File Manager — no shell
     * access required.
     */
    ...
    public function renderPage(int $code, string $message): string
    {
        ...
        // The "Back to Quizontal Cloud" button. Set the URL at the top of this
        // class (private static string $storefrontUrl); leave it empty to hide it.
        ...
        // $message is already escaped by ExceptionResponseFactory; the specialized
        // message from getCodes() carries trusted inline markup and is emitted raw.
        $storefrontButton = '';
        if ($storefrontUrl !== '') {
...
<head>
    ...
    <title>{$title} · Quizontal Cloud</title>
    <style>
        :root {
            --qc-bg: #050508;
            --qc-card: #101018;
        ...
        * { box-sizing: border-box; }
        ...
        .qc-error { width: 100%; max-width: 640px; text-align: center; }
        .qc-logo { height: 40px; margin-bottom: 28px; }
        .qc-code {
        ...
        .qc-message { color: var(--qc-text); margin: 0 0 6px; font-size: 1.02rem; }
        .qc-message code {
        ...
        .qc-original { color: var(--qc-muted); margin: 0 0 6px; font-size: 0.95rem; display: none; word-break: break-word; }
        .qc-actions { display: flex; flex-wrap: wrap; gap: 12px; justify-content: center; margin-top: 26px; }
        .qc-btn {
            display: inline-flex;
            align-items: center;
            justify-content: center;
        ...
        .qc-footer { margin-top: 22px; color: var(--qc-muted); font-size: 0.82rem; }
    </style>
</head>
<body>
    <div class="qc-error">
        ...
        <div class="qc-card">
            ...
            <div class="qc-actions">
                <a class="qc-btn qc-btn-primary" href="{$homeUrl}">Return to the client area</a>
                {$storefrontButton}
                <button class="qc-btn qc-btn-ghost" id="toggle" type="button" onclick="toggle()">Show original message</button>
                <a class="qc-btn qc-btn-ghost" target="_blank" rel="noopener" href="{$linkHref}">{$linkLabel}</a>
            </div>
            <div class="qc-meta">
                <span>Instance ID: {$instance}</span><span class="sep">·</span>
                <span>Error code: #{$displayCode}</span><span class="sep">·</span>
                <span>Component: {$category}</span>
            </div>
        </div>
        <div class="qc-footer">&copy; {$year} Quizontal Cloud — domains, hosting &amp; cloud VPS</div>
    </div>
    <script>
        function toggle() {
            ...
            if (og.style.display === "none") {
                ...
                btn.textContent = "Show specialized message";
            } else {
                og.style.display = "none";
                specialized.style.display = "block";
                btn.textContent = "Show original message";
            }
        }
    </script>
</body>
