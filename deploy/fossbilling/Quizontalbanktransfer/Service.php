...
class Service implements \FOSSBilling\InjectionAwareInterface
{
    ...
    private function sendReceiptStatusEmail(int $submissionId): void
    {
        $submission = $this->get($submissionId);
        $client = $this->di['db']->load('Client', (int) $submission['client_id']);
        if (!$client instanceof \Model_Client) return;
        $balance = (float) $this->di['mod_service']('Client', 'Balance')->getClientBalance($client);
        $customer = $this->di['mod_service']('Client')->toApiArray($client);
        $this->di['mod_service']('Email')->sendTemplate([
            'to_client' => (int) $client->id,
            'code' => 'mod_quizontalbanktransfer_receipt_status',
            'submission' => $submission,
            'customer' => $customer,
    ...
    public function reject(int $id, string $note): bool
    {
        ...
        try { $this->sendReceiptStatusEmail($id); } catch (\Throwable $emailError) { error_log('Wallet rejection email failed: '.$emailError->getMessage()); }
        return true;
    }
