...
// ---------------------------------------------------------------------------
// Quizontal Cloud managed override of FOSSBilling 0.7.1 stock
// src/library/Server/Manager/Directadmin.php
// Behaviour is identical to stock; the only changes are brand-safe,
// white-label error messages (no supplier names, command names, URLs or
// HTTP details reach end users). Full technical reasons are written to the
// application log (data/log/application.log). Restore the stock file to revert.
// ---------------------------------------------------------------------------
...
class Server_Manager_Directadmin extends Server_Manager
{
    ...
    public function init(): void
    {
        ...
        if (empty($this->_config['host'])) {
            throw new Server_Exception('The ":server_manager" server manager is not fully configured. Please configure the :missing', [':server_manager' => 'hosting', ':missing' => 'hostname'], 2001);
        }

        // Check if username is set in the configuration
        if (empty($this->_config['username'])) {
            throw new Server_Exception('The ":server_manager" server manager is not fully configured. Please configure the :missing', [':server_manager' => 'hosting', ':missing' => 'username'], 2001);
        }

        // Check if password is set in the configuration
        if (empty($this->_config['password'])) {
            throw new Server_Exception('The ":server_manager" server manager is not fully configured. Please configure the :missing', [':server_manager' => 'hosting', ':missing' => 'password'], 2001);
        }
    }
    ...
    {
        throw new Server_Exception(__trans('The main domain of a hosting account cannot be changed after creation. Please open a support ticket if you need to move your website to a different domain.'));
    }
    ...
    {
        throw new Server_Exception(__trans('IP address changes are not available for hosting accounts.'));
    }
    ...
    {
        throw new Server_Exception(__trans('Usernames cannot be changed after the hosting account is created.'));
    }
    ...
    public function createAccount(Server_Account $account): bool
    {
        ...
        if (str_contains(implode('', $results), 'Error Creating User')) {
            throw new Server_Exception(__trans('The hosting account could not be created. Please open a support ticket and our team will set it up for you.'));
        }
    ...
    {
        throw new Server_Exception(__trans('Usage synchronization is not available for this hosting account.'));
    }
    ...
    public function unsuspendAccount(Server_Account $account): bool
    {
        ...
        if ($info['suspended'] == 'no') {
            throw new Server_Exception(__trans('This hosting account is not suspended.'));
        }
    ...
    private function request(string $command, array $fields = [], bool $post = true, string $asUser = ''): array
    {
        ...
        try {
            ...
            // Keep the full transport details (URLs, HTTP codes) in the application
            // log for the administrator only - never echo them to users.
            $this->getLog()->err('Hosting server transport error: ' . $error->getMessage());

            throw new Server_Exception(__trans('We could not reach the hosting server to complete this action. Please try again in a few minutes. If the problem continues, please open a support ticket.'));
        }
        ...
        if (strlen(strstr($data, '<!doctype html>')) > 0 || strlen(strstr($data, 'DirectAdmin Login')) > 0) {
            throw new Server_Exception(__trans('Failed to connect to the hosting server. Please verify the server credentials and configuration in the hosting settings.'));
        }
        ...
        if (strlen(strstr($data, "The request you've made cannot be executed because it does not exist in your authority level")) > 0) {
            throw new Server_Exception(__trans('The hosting server rejected this action because the configured API credentials do not allow it. Please review the server login key and its allowed commands.'));
        }
        ...
        if (isset($response['error']) && $response['error'] == 1) {
            // The exact reason from the hosting server stays in the application log
            // for the administrator; users only get a neutral, brand-safe message.
            $this->getLog()->err('CMD_' . $command . ' rejected by the hosting server: ' . ($response['text'] ?? '') . ': ' . ($response['details'] ?? ''));

            throw new Server_Exception(__trans('We could not apply this change to the hosting account. Please review the submitted details and try again. If the problem continues, please open a support ticket.'));
        }
    ...
    private function getUserPackages(): array
    {
        ...
        if (isset($results['error']) && $results['error'] == 1) {
            $this->getLog()->err('Failed to retrieve hosting packages from the server: ' . ($results['text'] ?? '') . ': ' . ($results['details'] ?? ''));
            throw new Server_Exception(__trans('The hosting package list could not be loaded from the server. Please try again shortly or check the server configuration.'));
        }
